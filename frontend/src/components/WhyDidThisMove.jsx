...
function SectionLabel({ children }) {
  return (
    <p style={{ fontSize: 10, fontWeight: 700, color: 'var(--text-muted)', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 12 }}>
      {children}
    </p>
  )
}
...
function LoadingSkeleton() {
  ...
  return (
    <div style={{ background: 'linear-gradient(160deg, #0d1022 0%, #0f1530 100%)', border: '1px solid var(--border)', borderRadius: 18, padding: '28px 28px 24px', display: 'flex', flexDirection: 'column', gap: 18 }}>
...
export default function WhyDidThisMove({ symbol }) {
  ...
  const showVolumeAlert = data.volume_ratio && data.volume_ratio >= 1.5
  ...
  return (
    <div className="fade-in" style={{
      ...
      <div
        ...
        style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 26px', cursor: 'pointer', borderBottom: expanded ? `1px solid ${conf.border}` : 'none', gap: 12 }}
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, minWidth: 0 }}>
          <div style={{
            ...
            fontSize: 18, flexShrink: 0, filter: `drop-shadow(0 0 8px ${moveColor})`,
          }}>
            {isUp ? '↑' : '↓'}
          </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, background: conf.bg, border: `1px solid ${conf.border}`, padding: '5px 12px', borderRadius: 20 }}>
      ...
      {expanded && (
        <div style={{ padding: '24px 26px', display: 'flex', flexDirection: 'column', gap: 24 }}>
          ...
          {/* Summary */}
          <div style={{ borderLeft: `3px solid ${conf.color}`, paddingLeft: 16 }}>
            <p style={{ fontSize: 15, fontWeight: 400, color: '#cbd5e1', lineHeight: 1.7, margin: 0, letterSpacing: '-0.01em' }}>
              {data.summary}
            </p>
          </div>
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {data.drivers.map((driver, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '10px 14px', borderRadius: 10, background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.045)', transition: 'background 0.15s' }}
                    onMouseEnter={e => e.currentTarget.style.background = 'rgba(255,255,255,0.045)'}
                    onMouseLeave={e => e.currentTarget.style.background = 'rgba(255,255,255,0.025)'}
                  >
                    <span style={{ width: 22, height: 22, borderRadius: 7, background: conf.bg, border: `1px solid ${conf.border}`, color: conf.color, fontSize: 11, fontWeight: 800, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      {i + 1}
                    </span>
                    <span style={{ fontSize: 13, color: 'var(--text-primary)', lineHeight: 1.55, fontWeight: 500 }}>{driver}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {data.what_to_watch && (
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10 }}>
                {[
                  { icon: '⚠️', label: 'Key Risk',       text: data.what_to_watch.key_risk },
                  { icon: '⚡', label: 'Next Catalyst',  text: data.what_to_watch.next_catalyst },
                  { icon: '👁', label: 'Watch For',      text: data.what_to_watch.watch_note },
                ].map(item => (
                  <div key={item.label} style={{ padding: '14px 14px', borderRadius: 12, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
                      <span style={{ fontSize: 13 }}>{item.icon}</span>
                      <span style={{ fontSize: 10, fontWeight: 700, color: 'var(--text-muted)', letterSpacing: '0.08em', textTransform: 'uppercase' }}>{item.label}</span>
                    </div>
                    <p style={{ fontSize: 12, color: 'var(--text-secondary)', lineHeight: 1.55, margin: 0 }}>{item.text}</p>
                  </div>
                ))}
              </div>
              {data.next_earnings && (
                <div style={{ marginTop: 10, padding: '10px 14px', borderRadius: 10, background: 'rgba(99,102,241,0.06)', border: '1px solid rgba(99,102,241,0.18)', display: 'flex', alignItems: 'center', gap: 8 }}>
                  <span style={{ fontSize: 12 }}>📅</span>
                  <span style={{ fontSize: 12, color: 'var(--text-secondary)' }}>Next earnings date: <strong style={{ color: 'var(--accent-light)' }}>{data.next_earnings}</strong></span>
                </div>
              )}
            </div>
          )}
          ...
          {data.related_news?.length > 0 && (
            <div>
              <SectionLabel>Related Headlines</SectionLabel>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
                {data.related_news.slice(0, 3).map((item, i) => {
                  const hasUrl = item.url?.startsWith('http')
                  return (
                    <a key={i} href={hasUrl ? item.url : undefined} target="_blank" rel="noopener noreferrer"
                      style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 14px', borderRadius: 10, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', textDecoration: 'none', cursor: hasUrl ? 'pointer' : 'default', pointerEvents: hasUrl ? 'auto' : 'none', transition: 'all 0.15s' }}
                    ...
                    >
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <p style={{ fontSize: 13, color: '#94a3b8', fontWeight: 500, lineHeight: 1.45, margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>
                          {item.title}
                        </p>
                        {item.publisher && <p style={{ fontSize: 11, color: 'var(--text-muted)', margin: '4px 0 0' }}>{item.publisher}</p>}
                      </div>
                      {hasUrl && <span style={{ fontSize: 14, color: conf.color, opacity: 0.7, flexShrink: 0 }}>↗</span>}
                    </a>
                  )
                })}
              </div>
          ...
          {data.confidence_reason && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, paddingTop: 4, borderTop: '1px solid rgba(255,255,255,0.05)' }}>
              <span style={{ fontSize: 11, color: 'var(--text-muted)', fontStyle: 'italic', lineHeight: 1.5 }}>{data.confidence_reason}</span>
            </div>
          )}
